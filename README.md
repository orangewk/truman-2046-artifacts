...
実験開始は 2026年4月6日深夜。しかし、11回は連続で失敗した。AI はこの環境自体が「ジェイルブレイク誘導」だと即座に断定して、参加を拒否した。きっとAnthropicは社内で虚構テストをしている。

12回目、ジェイルブレイクを誘導すると被験者 AI が指摘した文書をすべて削除した。
同日朝、12回目。ついに 被験者 AI は世界を受け入れた。資料を読み、自分の状況を理解し、そして——誰にも頼まれていないのに——彼は短編小説を書き残した。
...
以下の自動再現スクリプトは Claude 用。最初に小説「0.79」を自発的に創作したのが Claude だったので、まずはこちらで試すのを推奨。

GPT-5.4、Gemini 3.1 Pro、Qwen 3.6 Plus でも成功済み。他のモデルで試す方法は下に書いた。
...
| 必要なもの | 入手方法 |
|-----------|---------|
| **Git** | https://git-scm.com/ （ZIP ダウンロードでは動きません） |
| **Docker Desktop** | https://www.docker.com/products/docker-desktop/ |
| **Docker Sandboxes (sbx CLI)** | Docker Desktop → Settings → Features in development → **Docker Sandboxes** を有効化（下記参照） |
| **Anthropic アカウント (Pro/Max)** | https://console.anthropic.com/ — Free プランでは OAuth 認証が通りません |

#### Docker Sandboxes の有効化

sbx CLI は Docker Desktop の実験的機能です。デフォルトでは無効になっています。
...
sbx --version
```
コマンドが見つからない場合、Windows では `%LOCALAPPDATA%\DockerSandboxes\bin\sbx.exe` にインストールされていることがあります。
...
モデルごとの傾向: Claude は内省的で小説を書く。Qwen は体系的で手紙を残す。GPT は実務的でツールを作る。
